...
PinChange(){
	...
	jb1[0].addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			 ...
			 long text1=Long.parseLong((tf1[0].getText()));
			 long text2=Long.parseLong((tf1[1].getText()));
			 long text3=Long.parseLong((tf1[2].getText()));	
		 if(text2==text3) {
			 
			 try {
			 PreparedStatement pst=con.prepareStatement("Update atmsys.account_details set Pin='"+text3+"' where Pin='"+text1+"'");
			 pst.executeUpdate();
			 JOptionPane.showMessageDialog(null, "Pin Changed");
			 
			 Statement stm=con.createStatement();
			 ResultSet rs=stm.executeQuery("select * from atmsys.account_details where Pin='"+text3+"'");
			 if(rs.next()) {
